...
export default asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!newMessage) {
      return res.status(400).json({ success: false, message: 'Message not created..' });
    }
    ...
    await sendMailSparkPost({ template_id: `new-message-received` }, [
      {
        address: {
        ...
        substitution_data: {
          ...
          name: isUser ? ticket.requester?.fullName : ticket.assignee?.fullName,
          support_subject: ticket.subject,
          support_link: `https://portal.shepherdcms.org/account_details/support/${ticket._id}`,
        },
      },
    ]);
