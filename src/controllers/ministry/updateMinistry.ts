...
export default asyncHandler(async (req: AuthenticatedRequest, res: Response, next: any) => {
  try {
     const updatedMinistry = await Ministry.findByIdAndUpdate(req.params?.id, {...req.body}, { new: true, runValidators: true }).populate('leader');
    ...
    res.status(200).json({ success: true, ministry: updatedMinistry, message: `Ministry ${updatedMinistry.name} was successfully updated` });
  } catch (error: any) {
    console.log(error);
